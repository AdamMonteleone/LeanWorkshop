...
Worked solutions to the nineteen preparatory exercises and two main proofs.
...
lemma prime_divisor_factorial_add_one_contradiction (n p : ℕ)
    ...
    have hone := Nat.dvd_sub hdiv hfac
    simp at hone
    simp [hone]
...
it would also divide n!, hence n!+1-n! = 1, contradicting primality. Thus p > n.
-/
...
theorem primes_above_every_bound (n : ℕ) :
    ...
    apply prime_divisor_factorial_add_one_contradiction n p hp hle
    exact hdiv
...
lemma even_of_square_eq_two_mul_square (a b : ℤ) (hsq : a ^ 2 = 2 * b ^ 2) :
    (2 : ℤ) ∣ a := by
    apply even_of_even_square
    exact two_dvd_square_of_eq_two_mul_square a b hsq
...
theorem sqrt_two_by_parity : Irrational (Real.sqrt 2) := by
    ...
    -- By 3(c), a² = 2b² implies that a is even.
    have ha_even : (2 : ℤ) ∣ a := by
        exact even_of_square_eq_two_mul_square a (b : ℤ) hsq_int
    -- Pass to |a| so that this fact can be used with the natural-number gcd.
    have ha_abs : 2 ∣ a.natAbs := toNatAbs a ha_even
    obtain ⟨k, hk⟩ := ha_even  -- Unpack 2 ∣ a: hk is a = 2*k for some integer k.
    -- Use 3(d) to substitute a = 2k and obtain b² = 2k².
    have hb_sq : (b : ℤ) ^ 2 = 2 * k ^ 2 := by
        exact denominator_square_after_substitution a (b : ℤ) k hsq_int hk
    -- Apply 3(c) again, this time to b and k, to show that b is even.
    have hb_even : (2 : ℤ) ∣ (b : ℤ) := by
        exact even_of_square_eq_two_mul_square (b : ℤ) k hb_sq
    -- Now |a| and b are both even, contradicting their coprimality by 3(e).
    exact coprime_not_both_even a.natAbs b hcoprime
        ha_abs (toNat b hb_even)
